...
bool mark[maxn];
vector<vector<int> > gr1(maxn);
vector<vector<int> > gr2(maxn);
long long ans;
int n, num_mark;
...
class TreesAnalysis {
 public:
	...
	pair<int, int> dp(int u, int fa) {
		int sum_in_mark = mark[u];
		int sum_in_tree = 1;
		for (int t = 0; t != gr2[u].size(); t++)
			if (gr2[u][t] != fa) {
				pair<int, int> tmp = dp(gr2[u][t], u);
				sum_in_mark += tmp.first;
				sum_in_tree += tmp.second;
			}
		if (fa >= 0) {
			ans += sqr((long long)(max4(sum_in_mark, 
																	sum_in_tree - sum_in_mark,
																  num_mark - sum_in_mark, 
																	n - num_mark - sum_in_tree + sum_in_mark)));
		}
		return make_pair(sum_in_mark, sum_in_tree);
	}
  ...
  long long treeSimilarity(vector <int> tree1, vector <int> tree2) {
  	n = tree1.size() + 1;
		...
		for (int i = 0; i != n-1; i++) {
			gr1[i].push_back(tree1[i]);
			gr1[tree1[i]].push_back(i);
			gr2[i].push_back(tree2[i]);
			gr2[tree2[i]].push_back(i);
		}
